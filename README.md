...
9. Medium uses a seeded **Threat Budget** that grows 25% per level, rounded up. When enabled, building health bars are green at full health, yellow below full, and red below 30%; their length shows the exact amount. Ending a day with unused actions presents one confirmation per day. The compact Night Record retains spawned enemies, defense damage, and homestead damage for each watch. Playback speed is a saved 1×/2× preference that may be changed any time after shelter construction and only changes simulation pacing at night.
10. The interface keeps one objective prompt at the top and no terrain inspector below the map. Day actions are separate from a compact horizontal **Build** strip: it stays empty during the forced shelter opening, begins with the unlocked Stick Launcher after Level 1, and adds later unlocked structures (such as the Potato Gun) from left to right as square, icon-first tiles. Its saved − / + control selects Compact, Standard, or Large sizing. At night both rails remain in place but build controls are disabled; rapid combat narration stays off-screen. Technology opens as a full-screen overlay and shows available Skill Points in the header.
