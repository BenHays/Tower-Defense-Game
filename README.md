...
3. Every normal day has two actions. **Harvest Tree**, placing Scout, building a tower, repairing, and upgrading each use **one** action. Harvesting grants **2 wood** and turns the tree into open grass; it has a short hatchet-and-wood-chip animation. Scout's watch post reserves its cell: towers cannot overlap it, and Scout cannot be placed inside a structure.
...
10. The interface keeps one objective prompt at the top and no terrain inspector below the map. Day actions are separate from a compact horizontal **Build** strip: it stays empty during the two-action shelter opening, begins with the unlocked Stick Launcher after Level 1, and adds later direct structures (such as the Potato Patch) from left to right as square, icon-first tiles. A mature selected patch exposes its Potato Gun conversion in the contextual upgrade row. Its saved − / + control selects Compact, Standard, or Large sizing. At night both rails remain in place but build controls are disabled; rapid combat narration stays off-screen. Technology opens as a full-screen, horizontally scrollable dependency canvas: each card shows its concise effect, cost, and relationship before the player selects it.
