...
9. Medium uses a seeded **Threat Budget** that grows 25% per level, rounded up. Building condition uses a color dot: green at full health, yellow below full, and red below 30%; optional health bars show the exact amount. The compact Night Record retains spawned enemies, defense damage, and homestead damage for each watch. Playback speed is a saved 1×/2× preference that may be changed any time after shelter construction and only changes simulation pacing at night.
