...
3. Every normal day has two actions. Clearing a tree, placing Scout, building a tower, repairing, and upgrading each use **one** action. Clearing grants **2 wood** and turns the tree into open grass. Scout's watch post reserves its cell: towers cannot overlap it, and Scout cannot be placed inside a structure.
4. Level 1 unlocks the **Stick Launcher**. On Level 2, clear a tree and build it for 2 wood on any unoccupied grass cell in the same day. It has 8 health, deals 1 damage, fires once every 2 seconds, and has short reach.
...
6. Defeated enemies and held nights grant permanent **XP**. Every **8 XP** earns one spendable **Skill Point**; research consumes Skill Points only—never a day action. The first Skill Point opens a short Technology explanation. Raccoons roll 1–2 **Hides** and Boars roll 2–4; those seeded drops are held for a future trade/food/upgrade decision and do not yet have a cost sink.
7. Technology is a full-screen, data-driven tree with four branches: **Huntcraft** (Stick Launcher damage → range → Arrowcraft and refits), **Forager** (extra wood → stronger repairs), **Fortification** (dawn regeneration → maximum HP → armor), and **Scout** (damage → watch range). Dependencies and level gates are shown directly on the nodes. Upgrading a damaged Stick Launcher is still a paid refit: the Arrow Shooter starts at full health.
...
9. Medium uses a seeded **Threat Budget** that grows 25% per level, rounded up. When enabled, building health bars are green at full health, yellow below full, and red below 30%; their length shows the exact amount. **End Day** begins the night immediately, even if actions remain. The compact Night Record retains spawned enemies, defense damage, and homestead damage for each watch. Playback speed is a saved 1×/2× preference that may be changed any time after shelter construction and only changes simulation pacing at night.
10. The interface keeps one objective prompt at the top and no terrain inspector below the map. Day actions are separate from a compact horizontal **Build** strip: it stays empty during the forced shelter opening, begins with the unlocked Stick Launcher after Level 1, and adds later unlocked structures (such as the Potato Gun) from left to right as square, icon-first tiles. Its saved − / + control selects Compact, Standard, or Large sizing. At night both rails remain in place but build controls are disabled; rapid combat narration stays off-screen. Technology opens as a full-screen, horizontally scrollable dependency canvas: each card shows its concise effect, cost, and relationship before the player selects it.
