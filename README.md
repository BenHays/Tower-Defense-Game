...
10. The interface keeps one objective prompt at the top and no terrain inspector below the map. The lower dock always reads **Day Actions → Build → Settings**: the Build strip stays empty during the two-action shelter opening, begins with the unlocked Stick Launcher after Level 1, and adds later direct structures from level discovery or researched Talents as square, icon-first tiles. Settings always stay at the bottom and hold the saved control-size − / + controls, health-bar toggle, sound controls, planning overlay, and 1×/2×/5× playback preference. At night the Build strip stays in place but is disabled; rapid combat narration stays off-screen. The Talent Tree opens as a full-screen, two-axis-scrollable dependency canvas with compact icon-first nodes, curved branch links, and a selection-driven detail pane. Revealed nodes carry concise mechanical tags—such as **+1 Launcher DMG**, **Unlock Fence**, or **+1 HP / Dawn**—which repeat beside the selected talent.
