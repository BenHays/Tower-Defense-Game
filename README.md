...
9. Medium uses a seeded **Threat Budget** that grows 25% per level, rounded up. Building condition uses a color dot: green at full health, yellow below full, and red below 30%; optional health bars show the exact amount. Ending a day with unused actions presents one confirmation per day. The compact Night Record retains spawned enemies, defense damage, and homestead damage for each watch. Playback speed is a saved 1×/2× preference that may be changed any time after shelter construction and only changes simulation pacing at night.
10. The interface keeps one objective prompt at the top. The Field Kit is a compact bottom toolbar for day actions; its saved − / + control selects Compact, Standard, or Large sizing. At night it remains in place as a disabled orientation rail while only watch controls are interactive; rapid combat narration stays off-screen. Technology opens as a full-screen overlay.
