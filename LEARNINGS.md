...
- Day Actions and Build must have a hard single-row vertical budget: present their controls as icon rails and scroll horizontally when the roster grows, including repair/upgrade controls that only appear after a structure is selected. Keep the same map → Day Actions → Build order at every viewport width; desktop space may widen rails but must never turn them into a side dock.
